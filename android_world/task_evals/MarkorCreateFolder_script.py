...
class TaskOperationExecutor:
    ...
    def run_operations(self, env: AsyncAndroidEnv) -> None:
        ...
        # -------------------------- 步骤2：打开Markor APP --------------------------
        logging.info("📱 步骤1/11：打开Markor APP")
        ...
        try:
            self._click_by_index(
                env=env,
                index=50,
                step_desc="点击坐标(原始:(29308,28220) → 屏幕:(965,2066))"
            )
        except IndexError:
            ...
            self._click_by_coords(
                ...
                x=965,
                y=2066,
                step_desc="点击坐标(原始:(29308,28220) → 屏幕:(965,2066))（索引无效，坐标兜底）"
            )
        ...
        try:
            self._click_by_index(
                env=env,
                index=0,
                step_desc="点击坐标(原始:(7251,7481) → 屏幕:(238,547))"
            )
        except IndexError:
            ...
            self._click_by_coords(
                ...
                x=238,
                y=547,
                step_desc="点击坐标(原始:(7251,7481) → 屏幕:(238,547))（索引无效，坐标兜底）"
            )

        # 步骤 3/6：WAIT
        logging.info("⌛ 等待3.89秒")
        time.sleep(3.89)
        ...
        self._input_text(
            env=env,
            text="20152213",
            step_desc="输入文本20152213\n"
        )

        # 步骤 5/6：WAIT
        logging.info("⌛ 等待5.61秒")
        time.sleep(5.61)
        ...
        try:
            self._click_by_index(
                env=env,
                index=10,
                step_desc="点击坐标(原始:(27882,15182) → 屏幕:(925,1117))"
            )
        except IndexError:
            logging.warning(f"元素索引8无效，切换为坐标点击")
            self._click_by_coords(
                env=env,
                x=925,
                y=1117,
                step_desc="点击坐标(原始:(27882,15182) → 屏幕:(925,1117))（索引无效，坐标兜底）"
            )
